...
# ------------------------------------------------------------------------------
resultados_carregados <- fs::dir_ls("data-raw/results_multivariate/") %>%
  purrr::map_dfr(readr::read_rds)

# tabelas resumo ---------------------------------------------------------------
# deixei explicito para ficar facil de editar, caso queira

fazer_tabelas <- function(a) {
  true <- tibble::tribble(
    ~key, ~p, ~true_value,
    "d", 1, .4,
    "d", 2, .4,
...
entre <- function(x, left, right) {
  mean(purrr::map2_lgl(left, right, ~dplyr::between(x[1], .x, .y)))
}

tabelas <- resultados_carregados$result %>%
  purrr::map_dfr(fazer_tabelas, .id = "rep_id") %>%
  dplyr::group_by(key, p, is_alpha, true_value) %>%
  dplyr::summarise(
    n_rep = n(),
    media_mediana = mean(mediana),
    media_sd = mean(sd),
    prop_intervalo = entre(true_value, q025, q975)
  ) %>%
  dplyr::ungroup() %>%
